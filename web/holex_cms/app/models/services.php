...
class services
{
    ...
    public function getServicesList()
    {
        ...
        if ($pages_amount > 0) {
            ...
            $sqlGetServices = "SELECT p.*, tr.text AS title FROM `" . $this->table . "` p " . implode("\n", $joins) . "
    ...
    public function addService()
    {
        ...
        if (!empty($_FILES['img']['name'])) {
            if (empty($_FILES['img']['error'])) {
                $service['image'] = utils::upload($_FILES['img']['name'], $_FILES['img']['tmp_name'], UPLOADS_DIR.'clients/', images::$allowed_ext);
                if (empty($service['image'])) {
                    $response['errors'][] = 'upl_invalid_image_extension_err';
                } else {
                    @mkdir(UPLOADS_DIR.'clients/', 0777, true);
                    $img = new SimpleImage(UPLOADS_DIR.'clients/'.$service['image']);
                }
            } else {
                $response['errors'][] = CMS::$upload_err[$_FILES['img_az']['error']];
            }
        }
        // saving translates
