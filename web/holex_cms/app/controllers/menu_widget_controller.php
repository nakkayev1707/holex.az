...
//		],
        [
            ...
            'icon' => 'files-o',
            'selected' => ['news/list', 'articles/list', 'articles/add', 'articles/edit', 'articles/delete', 'gallery/list', 'gallery/add', 'gallery/edit', 'gallery/delete', 'gallery/photos'],
            'subs' => [
                'news/list' => [
    ...
    { // 2017-04-06
        ...
        foreach (self::$menu as $i => $block) {
            ...
            if (empty($block['selected'])) {
                $block['selected'] = array_keys(@$block['subs']);
            }
            if (empty($block['icon'])) {
                $block['icon'] = 'circle-o';
            }
            ...
            if (!empty($block['subs'])) {
                ...
                foreach ($block['subs'] as $page => $item) {
                    if (!CMS::hasAccessTo($page)) {
                        continue;
                    }

                    if (empty($item['name'])) {
                        $item['name'] = 'menu_item_' . str_replace('/', '_', $page);
                    }
